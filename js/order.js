...
cartNumber.addEventListener("click", function (e) {
  ...
  editCart.forEach((item, index) => {
    item.addEventListener("click", (e) => {
      ...
      let itemNoSelected = [...editCart].filter(notSelect=>!notSelect.classList.contains("selected"));
      itemNoSelected.forEach(a=>a.parentNode.parentNode.classList.add("display--none"));
      let newTotal = e.target.parentNode.closest(".item__body").childNodes[1];
      let editShoppingCart = e.target.parentNode.closest(".item__info").childNodes[1].childNodes[7];
      ...
      minus.forEach((minusItem, minusIndex) => {
        minusItem.addEventListener("click", (e) => {
          if (numberUpdate > 1) {
            ...
            newTotal.textContent = `Tổng: ${Intl.NumberFormat().format(numberUpdate * shoppingList[index].price)}đ`;
          }
        });
      });
      save.forEach((saveItem) => {
        saveItem.addEventListener("click", (e) => {
          ...
          shoppingList[index].totalPrice = numberUpdate * shoppingList[index].price;
          document.querySelectorAll(".item__qnt")[index].textContent = `x${shoppingList[index].number}`;
          editShoppingCart.classList.add("display--none");
          item.classList.remove("selected");
          itemNoSelected.forEach(a=>a.parentNode.parentNode.classList.remove("display--none"));
        });
      });
    });
  });
  ...
  buyConfirm = document.querySelector(".buy__confirm");
  buyConfirm.addEventListener("click",function(){
    console.log(adviceCount);
    if(adviceCount ==0){
      const adviceAlert = window.confirm("Bạn có muốn nhận lời khuyên từ bác sĩ không?")
      if(adviceAlert){
        adviceCount++;
        window.open("https://www.youtube.com/",'_blank');
      } 
      else console.log(getCartList());
    }
    else console.log(getCartList());
  })
});
